...
{
    internal class OrderingQuestionXmlWriter : QuestionXmlWriter
    {
        ...
        public override void WriteXml(XmlTextWriter xmlWriter)
        {
            ...
            xmlWriter.WriteAttributeString("type", "order");
            ...
            if (question.ResponseVariants != null)
            {
                foreach (ResponseVariant rv in question.ResponseVariants)
                {
                    ...
                    {
                        for (var j = 0; j < question.Responses.Count; j++)
                        {
                            xmlWriter.WriteStartElement("checked");
                            xmlWriter.WriteAttributeString("answer_id", ((Response)question.Responses[j]).NativeId);
                            xmlWriter.WriteAttributeString("value", ((int)rv.Responses[j]).ToString());
                            xmlWriter.WriteFullEndElement();
                        }
                    }
