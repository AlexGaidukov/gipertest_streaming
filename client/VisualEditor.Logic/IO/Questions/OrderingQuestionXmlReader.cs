...
{
    internal class OrderingQuestionXmlReader : QuestionXmlReader
    {
                ...
                while (!isEndCycle && xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        if (xmlReader.Name.Equals("answer_variants"))
                        {
                            ...
                            //((ResponseVariant)responseVariants[responseVariants.Count - 1]).Type = reader.GetAttribute("type");
                            ...
                            // заменяем точки и запятые на действующия в системе разделитель, чтобы не было проблем с преобразованием в double
